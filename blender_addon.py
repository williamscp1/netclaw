...
class BlenderMCPServer:
    def __init__(self, host='localhost', port=9876):
        ...
        self.command_queue = Queue()  # Thread-safe queue for commands
    ...
    def start(self):
        ...
        try:
            ...
            # Register persistent timer to process command queue from main thread
    ...
    def _process_queue(self):
        """Process pending commands from queue - runs in main Blender thread"""
        try:
            # Process up to 10 commands per tick to avoid blocking
            for _ in range(10):
                try:
                    command, client, callback = self.command_queue.get_nowait()
                    cmd_type = command.get('type', 'unknown')
                    print(f"[MCP-QUEUE] Processing: {cmd_type}")
                    try:
                        response = self.execute_command(command)
                        response_json = json.dumps(response)
                        print(f"[MCP-QUEUE] Response ready ({len(response_json)} bytes)")
                        try:
                            client.sendall(response_json.encode('utf-8'))
                            print(f"[MCP-QUEUE] Response sent OK")
                        except Exception as e:
                            print(f"[MCP-QUEUE] Send failed: {e}")
                    except Exception as e:
                        print(f"[MCP-QUEUE] Execute error: {str(e)}")
                        traceback.print_exc()
                        try:
            ...
            print(f"[MCP-QUEUE] Processor error: {e}")

        # Return interval for next check (0.05s = 50ms)
        if self.running:
            return 0.05
        else:
            self.timer_registered = False
            ...
            while self.running:
                ...
                try:
                    ...
                    try:
                        ...
                        # Queue command for execution in main thread (thread-safe)
                        self.command_queue.put((command, client, None))
                        print(f"Command queued: {command.get('type', 'unknown')}")
    ...
    def execute_code(self, code):
        ...
        try:
            # Normalize render engine names for Blender version compatibility
            # BLENDER_EEVEE_NEXT was used in Blender 4.2, but 5.x uses BLENDER_EEVEE
            available_engines = [e.identifier for e in bpy.types.RenderEngine.__subclasses__()]
            if "BLENDER_EEVEE_NEXT" not in available_engines:
